...
TaskPilot operates only on explicitly allowed repositories. Owner-scoped opaque-token and OIDC
authentication isolate control-plane records in shared deployments, but the service is not a hostile
tenant boundary: graph workers and guarded writes retain repository access. Host validation runs
with the TaskPilot operating-system identity; container command mode isolates validation processes,
while hostile repositories require isolation around the whole service.
...
| Environment/secret leakage                | Child environment is reduced to process/runtime path variables; provider keys are excluded and tested                                                                             | Repository code can still read files accessible to the OS user. Use whole-service isolation for untrusted code.                                           |
...
| Invalid/binary source                     | UTF-8-only reads and bounded binary-safe hashing                                                                                                                                  | Non-UTF-8 source files are skipped and cannot be edited.                                                                                                  |
